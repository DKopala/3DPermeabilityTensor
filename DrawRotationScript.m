% This is simple script drawing H and B vectors for both models
...
subplot(2,2,2)
q2 = quiver3(0,0,0,B2(1),B2(2),B2(3),'r','LineWidth',4);
set(gca,'fontsize',20);
title('{\it B_{sat}} model with rotation (T)');
xlabel('{\it B_x} (T)');
ylabel('{\it B_y} (T)');
zlabel('{\it B_z} (T)');
...
axis([-1 1 -1 1 -1 1]);
hold on;
q4 = quiver3(0,0,0,vB2(1),vB2(2),vB2(3),'r','LineWidth',4);
hold off;
grid on;
...
set(gca,'fontsize',20);
title('Saturation tensor of {\it \mu} (-) with rotation');
xlabel('{\it x} (-)');
ylabel('{\it y} (-)');
zlabel('{\it z} (-)');
